...
export default {
  ...
  watch:{
	  searchParams(){
		//   console.log(this.searchParams, "我是分控");
		this.getSearchData(JSON.parse(this.searchParams));
	  }
  },
  created() {
  ...
  methods: {
	  getSearchData(params){
		...
		// 这就是 搜索的时候   请求接口  渲染页面   先写死   有接口 再改
        if (params.tera == "西北区") {

            let d = ["青海", "甘肃", '宁夏', "新疆", "陕西"];
            let x = [10, 900, 800, 700, 60, 500];
            this.get_zhandian(d, x);
        } else if (params.tera == "华北区") {
            if(params.pro == '山西省'){
                let d = ["临汾", "运城", "太原", "大同", "洪洞"];
                let x = [100, 90, 80, 70, 600, 50];
                this.get_zhandian(d, x);
            }else if(params.pro == '北京'){
                let d = ["海淀", "丰台", "朝阳", "顺义", "大兴"];
                let x = [100, 90, 80, 70, 600, 50];
                this.get_zhandian(d, x);
            }else if(params.pro == '天津'){
                let d = ["静海", "静海", "静海", "静海", "静海"];
                let x = [100, 90, 80, 70, 600, 50];
                this.get_zhandian(d, x);
            }
  ...
  mounted() {
    ...
    let x = [100, 90, 80, 70, 600, 50, 100, 90, 80, 70, 600, 50];
